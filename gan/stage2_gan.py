...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_start,opt.niter):
        for bdx, data in enumerate(dataloader):
            ...
            # real log-loss
            real_target = torch.ones(real_decision.size(0), 1, device=real_decision.device)
            real_log_loss = criterion(real_decision, real_target)
            ...
            # fake log-loss
            fake_target = torch.zeros(fake_decision.size(0), 1, device=fake_decision.device)
            fake_log_loss = fake_weight * criterion(fake_decision, fake_target)
            ...
            # log-loss backwards
            fool_target = torch.ones(fool_decision.size(0), 1, device=fool_decision.device)
            fool_log_loss = criterion(fool_decision, fool_target)
            ...
            D_mean_err = real_log_loss.item()+fake_log_loss.item()
            G_mean_err = fool_log_loss.item()+l1_loss.item() if use_l1_loss else fool_log_loss.item()
            if use_l1_loss:
                log_file.write('G_err: ' + str(fool_log_loss.item()) + ' lambda*L1_err: ' + str(l1_loss.item()) + ' L1_err: ' + str(l1_loss.item()/l1_lambda) + ' D_mean_err: ' + str(D_mean_err) + ' G_grad_norm:' + str(mean_grad_norm(gNet.named_parameters())) + '\n')
            else:
