...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_start, opt.niter):
        for bdx, data in enumerate(dataloader):
            ...
            # real log-loss
            real_target = torch.ones(batch_size, 1, device=real_decision.device)
            ...
            # fake log-loss
            fake_target = torch.zeros(batch_size, 1, device=fake_decision.device)
            ...
            # log-loss backwards
            fool_target = torch.ones(batch_size, 1, device=fool_decision.device)
